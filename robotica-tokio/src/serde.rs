...
pub fn merge_yaml(a: Value, b: Value) -> Result<Value, Error> {
    ...
    match (a, b) {
        ...
        (a, b) => Err(Error::InvalidTypes(
            serde_yaml_ng::to_string(&a).unwrap_or_default(),
            serde_yaml_ng::to_string(&b).unwrap_or_default(),
        )),
    }
}
...
";
        ...
        let a = serde_yaml_ng::from_str::<Value>(a).unwrap();
        let b = serde_yaml_ng::from_str::<Value>(b).unwrap();
        let c = serde_yaml_ng::from_str::<Value>(c).unwrap();
